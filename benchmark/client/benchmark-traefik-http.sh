...
./wrk -d 1m -c 640 -t 64 -R 10000 --latency http://$MONOLAKE_BENCHMARK_PROXY_IP:8300/server2 > http-result-4c-traefik-tiny.txt
./wrk -d 1m -c 640 -t 64 -R 11000 --latency http://$MONOLAKE_BENCHMARK_PROXY_IP:8300/server3 > http-result-4c-traefik-small.txt
./wrk -d 1m -c 640 -t 64 -R 12000 --latency http://$MONOLAKE_BENCHMARK_PROXY_IP:8300/server4 > http-result-4c-traefik-medium.txt
./wrk -d 1m -c 640 -t 64 -R 12000 --latency http://$MONOLAKE_BENCHMARK_PROXY_IP:8300/server5 > http-result-4c-traefik-large.txt
